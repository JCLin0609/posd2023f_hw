...
public:
    ...
    void rename(string newName) override {
        ...
        size_t slash = this->path().rfind("/");
        newPath = this->path().substr(0, slash+1) + newName;

        this->setPath(newPath);

        for(auto it = _nodes.begin(); it != _nodes.end(); ++it){
            (*it)->setPath(newPath + "/" + (*it)->name());
            if(dynamic_cast<Folder*>(*it) != nullptr){
                (*it)->rename((*it)->name());
            }
        }
    }
    ...
    Iterator * createIterator(IteratorFactory* itFactory) override{
        return itFactory->create(this, _operationCount);
    }
...
private:
    ...
    protected:
        ...
        Node * cloneSortedNode(Node * node) {
            ...
            else if(dynamic_cast<Link *>(node)) {
